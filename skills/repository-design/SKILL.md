...
2. If it is missing, **do not create one automatically.** Proceed with this
   ...
   repository folder plus the shared base classes). Once done, you may **suggest**
   the user run **`ddd:create-profile`** to capture those conventions in a
   committed `.claude/ddd/repository-design.md` for next time — but only create
